<template>
  <a-layout class="view-layout">
      ...
      <a-row>
        <a-col>
          <a-statistic
            ...
            :title="
              $t(
                `View.TokenTransfer.SingleToMultiple.template.content.tokenBalance`,
              )
            "
          />
        </a-col>
    ...
    <a-layout-footer style="margin: 0; padding: 0">
      <a-row style="margin-bottom: 0 !important">
        <a-col flex="1">
          <a-alert
            show-icon
            type="info"
            ...
            :message="
              $t(
                'cun-kuan-shou-xian-de-zhang-hu-xu-yao-tong-guo-cun-kuan-xian-zhi-gong-neng-jin-hang-biao-ji-bing-shou-dong-shan-chu',
              )
            "
          />
        </a-col>
  ...
  computed: {
    ...
    amountPlaceholder() {
      ...
      if (this.selectedToken && this.selectedToken.length) {
        ...
        if (token) placeholder = formatNumber(token?.amount as string);
      }
    ...
    balance() {
      if (this.selectedToken && this.selectedToken.length) {
        if (!this.tokenType) {
          ...
          if (token) return new Decimal(token?.amount as string);
        } else {
          const token = this.resourcesOfSender?.nonFungible.find((token) => {
            return token.resourceAddress === this.selectedToken;
          });

          if (token) return new Decimal(token?.ids?.length as number);
        }
      }
    ...
    isExceed() {
      if (this.selectedToken && this.selectedToken.length) {
        const totalSendAmount = new Decimal(this.totalTokenAmount);

        if (this.balance.lessThan(totalSendAmount)) {
          return true;
        }
      }
