...
export default defineComponent({
  ...
  computed: {
        ...
        this.feeLock.length
          ? this.hasFields
            ? new Decimal(this.feeLock)
                .mul(this.maxFeePerccent)
                .toFixed(
                  new Decimal(this.feeLock).mul(this.maxFeePerccent).toString()
                    .length > 18
                    ? 18
                    : new Decimal(this.feeLock)
                        .mul(this.maxFeePerccent)
    ...
    isPreviewDone() {
      return this.previewFeeList.length === this.actulExecutionTimes;
    },
    isCommitDone() {
    ...
    maxFeePerccent() {
      if (this.feePercentList.length) {
        let maxPercent = this.feePercentList[0];

        this.feePercentList.forEach((p) => {
          if (maxPercent.lessThan(p)) {
            maxPercent = p;
          }
        });
  ...
  methods: {
    ...
    async sendTransaction(txMessage: string) {
      ...
      if (this.hasFields) {
        ...
        const feeLockCode = (index: number) => {
          return this.$t("View.ManifestExecute.script.feeLockCode", [
            this.feePayerAddress,
            new Decimal(this.feeLock)
              .mul(this.feePercentList[index])
              .toFixed(
                new Decimal(this.feeLock)
                  .mul(this.feePercentList[index])
                  .toString().length > 18
    ...
    async previewTransaction() {
      ...
      {
        let manifestStr = `\n${await this.replaceAll(this.fieldData[0])}`;
      ...
      if (this.hasFields) {
        ...
        for (let i = 0; i < realExecutionTimes; i++) {
          ...
          this.store.worker.postMessage({
            action: "ManifestExecute.preview",
            args: [
              manifestStr,
              currentEpoch,
              feePayerPrivateKey,
        ...
        try {
          ...
          for (let i = 0; i < this.actulExecutionTimes; i++) {
            this.previewFeeList.push({
              order: i,
              fee: "0",
            });
          }

          this.feeLockEstimate = new Decimal(result.fee)
